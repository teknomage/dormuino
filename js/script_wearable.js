...
function requestDevice() {
  ...
  .catch(error => {
	log('requestDevice Error: ' + error);
  });;
}

//---------------------------------------------------------------------------------
function onConnectDormioButtonClick() {
  return (bluetoothDevice ? Promise.resolve() : requestDevice())
  .then(connectDeviceAndCacheCharacteristics)
  .then(_ => {
    log('Connecting to Dormio data-stream...');
  ...
  .catch(error => {
    log('onConnectDormio Error: ' + error);
  });
}
...
function connectDeviceAndCacheCharacteristics() {
  ...
  .then(characteristic => {
    ...
    dataCharacteristic.addEventListener('characteristicvaluechanged', handleDormioIncomingData);
    dataCharacteristic.startNotifications();
  })
  .catch(error => {
    log('connectDeviceAndChartx Error: ' + error);
  });;
}
